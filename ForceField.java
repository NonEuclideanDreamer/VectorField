...
public class ForceField 
{
	...
	public static int[] scale=VectorField.scale;
	...
	public ForceField(String string) {
		if(string.equals("wave"))
		{
			vectorfield=new double[scale[0]][scale[1]][2];
			for(int i=0;i<scale[0];i++)
				for(int j=0;j<scale[1];j++)
					vectorfield[i][j]=new double[] {1*Math.cos(i*0*Math.PI/scale[0])*Math.sin(j*10*Math.PI/scale[1]),2*Math.cos(j*0*Math.PI/scale[1])*Math.sin(i*10*Math.PI/scale[0])};
		}
		if(string.equals("circle"))
		{
			...
			for(int i=0;i<scale[0];i++)
				for(int j=0;j<scale[1];j++) {
					vectorfield[i][j]=new double[] {(j-scale[1]/2)*.01,-(i-scale[0]/2)*.01};
				}
		}
	}
	public ForceField(Function[]f,int t0)
	{	double size=2.0*Math.PI;
//	System.out.println(f[0].print());System.out.println(f[1].print());
		vectorfield=new double[scale[0]][scale[1]][f.length];
		double t=(t0-720)*0.02;
		for(int i=0;i<scale[0];i++)
			for(int j=0;j<scale[1];j++) {
				double x=(i*2-scale[0])*size/scale[1],y=(j*2-scale[1])*size/scale[1];
				for(int k=0;k<f.length;k++)
					vectorfield[i][j][k]=f[k].evaluate(new double[] {x,y,t});
			}
	}
	...
	//***********************************************
	// Methods working with the vector field structure
	//***********************************************
	...
	public void hgust(double t, double x,double y, int r, double str)
	{
		if(t%2<1)
		{
			double sign;
			...
			System.out.println("swoosh");
			int yi,xi;
			for(int i=-r;i<r;i++) {int s=(int)Math.sqrt(r*r-i*i);
				for(int j=-s;j<s;j++)
			{
				xi=(int) ((x+j+scale[0])%scale[0]);
				yi=(int) ((y+i+scale[1])%scale[1]);
				double factor=0.5*(1+Math.cos(Math.PI*Math.abs(i*i+j*j)/r/r));
				vectorfield[xi][yi][0]+=factor*str*sign;
			}}
		}
	}
...
public double value(double x, double y,int d)
{
	int x0=(int)x%scale[0],x1=(x0+1)%scale[0],
		y0=(int)y%scale[1],y1=(y0+1)%scale[1];
	double xd=x-x0,yd=y-y0;
	
	return vectorfield[x0][y0][d]*(1-xd)*(1-yd)+vectorfield[x1][y0][d]*(xd)*(1-yd)+vectorfield[x0][y1][d]*(1-xd)*(yd)+vectorfield[x1][y1][d]*(xd)*(yd);
}
